...
mouth_histogram_by_emotion <- function(emotion_idx){
    indices <- info[info$emotion_idx == emotion_idx, 'Index']
    emotion <- as.character(info[info$emotion_idx == emotion_idx, 'emotion_cat'])[1]
    mouth_area <- function(idx){
    return(polyarea(x=c(mouth[[idx]][1,1], mouth[[idx]][2,1],mouth[[idx]][3,1],mouth[[idx]][4,1],mouth[[idx]][1,1]),y=c(mouth[[idx]][1,2], mouth[[idx]][2,2],mouth[[idx]][3,2],mouth[[idx]][4,2],mouth[[idx]][1,2])))
  
}
    mouth_area_distribution <- sapply(indices, mouth_area)
    #jpeg(paste(c('../output/hist', emotion_idx,"_", pt1, "_", pt2, '.jpg'), collapse = ''),
    #width = 500, height = 375)
    hist(mouth_area_distribution, xlab = "area", 
         main = paste(c(emotion, "mouth area"),collapse = " ")) 
    abline(v = mean(mouth_area_distribution))
    #dev.off()
}
...
#lefteye_area_dist_histogram <- function(){
  lapply(1:22, mouth_histogram_by_emotion)
