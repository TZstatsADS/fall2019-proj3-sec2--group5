...
nose_histogram_by_emotion <- function(emotion_idx){
    indices <- info[info$emotion_idx == emotion_idx, 'Index']
    emotion <- as.character(info[info$emotion_idx == emotion_idx, 'emotion_cat'])[1]
    nose_area <- function(idx){
    return(polyarea(x=c(nose[[idx]][1,1], nose[[idx]][2,1],nose[[idx]][3,1],nose[[idx]][4,1],nose[[idx]][1,1]),y=c(nose[[idx]][1,2], nose[[idx]][2,2],nose[[idx]][3,2],nose[[idx]][4,2],nose[[idx]][1,2])))
  
}
    nose_area_distribution <- sapply(indices, nose_area)
    #jpeg(paste(c('../output/hist', emotion_idx,"_", pt1, "_", pt2, '.jpg'), collapse = ''),
    #width = 500, height = 375)
    hist(nose_area_distribution, xlab = "area", 
         main = paste(c(emotion, "nose area"),collapse = " ")) 
    abline(v = mean(nose_area_distribution))
    #dev.off()
}
...
#lefteye_area_dist_histogram <- function(){
  lapply(1:22, nose_histogram_by_emotion)
